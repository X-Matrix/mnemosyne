...
impl WhisperTranscriber {
    pub async fn load(model_id: &str) -> Result<Self, Error> {
        ...
        let tokenizer =
            Tokenizer::from_file(tokenizer_path).map_err(|e| Error::model(e.to_string()))?;
    ...
    pub fn transcribe(&self, path: &Path) -> Result<String, Error> {
        ...
        let mel = Tensor::from_vec(mel_vec, (1usize, n_mels, total_frames), &self.device)
            .map_err(|e| Error::model(e.to_string()))?;
        ...
        let max_new = (self.config.max_target_positions.saturating_sub(4)).min(MAX_DECODE_TOKENS);
        ...
        while seek < total_frames {
            ...
            let mel_seg = if seg_len < N_FRAMES {
                ...
                .map_err(|e| Error::model(e.to_string()))?;
                Tensor::cat(&[&mel_seg, &pad], 2).map_err(|e| Error::model(e.to_string()))?
            } else {
                mel_seg
            };
            ...
            'decode: for step in 0..max_new {
                ...
                let hidden = ys
                    .narrow(1, seq_len - 1, 1) // (1, 1, d_model)
                    .map_err(|e| Error::model(e.to_string()))?;
                let logits = model
                    .decoder
                    .final_linear(&hidden) // (1, 1, vocab)
                    .map_err(|e| Error::model(e.to_string()))?;
            ...
            let seg_text = self
                .tokenizer
                .decode(&tokens[4..], true) // skip special tokens
                .map_err(|e| Error::model(e.to_string()))?;
    ...
    let bytes: &[u8] = match n_mels {
        ...
        _ => {
            return Err(Error::model(format!(
                "no mel filter bank for n_mels={n_mels}"
            )))
        }
    };
    Ok(bytes
